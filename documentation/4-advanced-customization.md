...
Tailor the repository to your project's needs using these advanced customizations.
...
If under **Customizations** it says "No" for your locale, that means Custom Speech is not supported and you must exclude all three types of training data. You can still this solution to test baseline Azure Speech models against test data and/or as new baseline models are released.
...
### Edit the branch triggers

Complete the following to configure a clean **master**:

1. Configure [Gitflow](https://www.atlassian.com/git/tutorials/comparing-workflows/gitflow-workflow) by creating a **develop** branch off of master.
1. Find the following YAML in both `test\speech-test-data-ci.yml` and `training\speech-train-data-ci-cd.yml`:
...
1. Again in `speech-train-data-ci-cd.yml`, find the following YAML:
...
1. Remove the testing and training data Git objects and add the Git LFS objects:
